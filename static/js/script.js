...
const shirkBtn = document.getElementById("shirk_btn");

let clickOffsetX = 200;
let clickOffsetY = 180;
let uploadFiles = [];
...
list.addEventListener("drop", e => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    reader.addEventListener("load", () => {
      itemMarkup(files[i], reader.result, offsetX, offsetY);
    });
  }
  droppable.classList.remove("is-over");
});
...
function onchangeUploadFile (file) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    reader.readAsDataURL(files[i]);
    reader.addEventListener("load", () => {
      itemMarkup(files[i], reader.result, clickOffsetX, clickOffsetY);
    });
  }
}
